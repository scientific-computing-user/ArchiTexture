...
DEFAULT_EXPERIMENT_ROOT = ROOT / "experiments" / "detexture_ade20k_eval_20260317" / "full_all"
DEFAULT_OUT = ROOT / "TextureSum2_paper" / "figures" / "fig_detexture_audit.png"
DEFAULT_IMAGE_ID = 11


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build a Figure-3-style ADE20K-selected audit figure.")
...
def main() -> int:
    ...
    if len(proposals) < 2:
        raise RuntimeError(f"Selected ADE20K case {image_id} does not have enough proposals for an audit figure.")
    ...
    if len(clean_props) < 2:
        raise RuntimeError(f"Selected ADE20K case {image_id} does not have enough clean proposals.")
    ...
    if consolidator.learned_models is None:
        raise RuntimeError("Expected a learned ADE20K-selected bundle for the audit figure, but no learned model was loaded.")
    ...
    if not processed_components:
        raise RuntimeError(f"Selected ADE20K case {image_id} produced no merged components.")
