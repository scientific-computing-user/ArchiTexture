...
\paragraph{Training regime and frozen components.} The proposal generator is frozen. This route does not retrain SAM, alter the prompt source, or optimize the bank with benchmark labels. What is learned instead is the decision stack above the bank. The natural-anchor Stage-A bundle used here is trained on 480 PTD-only synthetic samples, and its rescue bundle is trained on 256 PTD-only \texttt{repair\_mix} samples that inject explicit under-coverage cores and hard wrong-side decoys. The synthetic-anchor Stage-A bundle is trained on 1600 PTD-only MPEG7-shape synthetic layouts. The ADE20K crop complement, ControlNet bridge complement, and appendix CAID route use the fixed PTD-trained \texttt{small\_pre\_ring\_hi} Stage-A bundle on their released archives. This separation is why the route speaks directly to recoverability: it measures how much task value remains once proposal generation is treated as fixed evidence rather than as the object of adaptation.
